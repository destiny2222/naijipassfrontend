...
import { getBids, Bid, getBidCategories, BidCategory, createBid } from "@/src/services/bids/bids";
...
export default function MyBidsPage() {
  ...
  const fetchBids = async () => {
    ...
    try {
      const response = await getBids(); // Fetch all bids for now
  ...
  const handleModalSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.bidNumber || !formData.deadline || !formData.agency || !formData.categoryId) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setFormData({ title: "", bidNumber: "", deadline: "", agency: "", categoryId: "" });
          ...
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="mt-1.5 text-xs font-medium text-zinc-400">
                Manage your tender submissions and track active bid status.
              </p>
            </div>
            <button 
              onClick={() => setIsModalOpen(true)}
              className="flex h-11 items-center justify-center gap-2 rounded-xl bg-[#FF6B2B] px-5 text-xs font-bold text-white shadow-md shadow-[#FF6B2B]/10 hover:bg-[#E55F23] hover:-translate-y-0.5 transition-all"
            >
              ...
              <table className="w-full text-left text-sm text-zinc-600">
                <thead className="bg-zinc-50/50 text-xs font-bold uppercase text-zinc-400 border-b border-zinc-100">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-4">Agency</th>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {fetchingBids ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-sm font-medium text-zinc-400">
                        Loading your bids...
                      </td>
                    </tr>
                  ) : bids.length === 0 ? (
                      ...
                      <tr key={bid.id} className="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${
                            isPastDeadline ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"
                          }`}>
                            {isPastDeadline ? "Closed/Awarding" : "Active"}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <button className="text-xs font-bold text-[#FF6B2B] hover:text-[#E55F23]">View</button>
                        </td>
                      </tr>
                    );
                  })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
              <h2 className="text-lg font-bold text-[#101D2D]">Submit New Bid</h2>
